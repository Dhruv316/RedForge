...
public class Store {
    ...
    public Store(){ map = new ConcurrentHashMap<>(); }

    public Set<String> getKeys(){
        rwLock.readLock().lock();
        try{ return map.keySet(); }finally{ rwLock.readLock().unlock(); }
    }
    ...
    public void executeTransaction(Client client, BiFunction<String[], Map<String, Value>, String> transactionCacheApplier){
        ...
        try{
            ...
            for(Map.Entry<String, Value> entry : localCache.entrySet()){
                ...
                if(value.isDeletedInTransaction) this.map.remove(key); else this.map.put(key, value);
            }
